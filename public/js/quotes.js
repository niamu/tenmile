...
* \`_.bin\` and \`_.mask\`: Similar to the files above, these contain information about various other dynamic memories. See emulator source for their proper usage.

* \`initialState.msgpack\`: A [MessagePack](https://github.com/msgpack/msgpack-javascript) encoded savestate for a [specific Game Boy emulator](https://github.com/rauchg/gameboy). After decoding, entry 0 of the resulting array should be replaced with a reference to the contents of the ROM image above. A few more entries for other memories should be replaced as well. Finally, entry 71 should be replaced with a reference to a 160 * 144 entry Int32Array with the decoded contents of the encloding PNG file (representing the screen visible at the start of the recorded actions). You should crop out just the 160x140 relevant pixels of the initial screenshot.
...
function generateMaskedMemory(
  ...
  for (let address of dependencies) {
    pages.add(Math.floor(address / PAGE_SIZE_IN_BYTES));
  }
  for (let page of pages) {
    let startAddress = page * PAGE_SIZE_IN_BYTES;
    for (let i = 0; i < PAGE_SIZE_IN_BYTES; i++) {
...
async function loadQuote(buffer) {
  ...
  quote.state = msgpack.deserialize(
    await zip.file("initialState.msgpack").async("uint8array")
  );
  quote.masks = {};
  for (let [e, { state_slot }] of Object.entries(SLICED_MEMORIES)) {
    quote.state[state_slot] = await zip.file(e + ".bin").async("uint8array");
    quote.masks[e] = await zip.file(e + ".mask").async("uint8array");
  }

  quote.state[SAVESTATE_FRAMEBUFFER] = frameBuffer;
  quote.actions = msgpack.deserialize(
...
export { loadQuote, compileQuote, Quote, Trace, SLICED_MEMORIES, BORDER_SIZE };